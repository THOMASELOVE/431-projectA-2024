...
- the [Examples material](examples.qmd) related to the Project A Plan, including the Project A Plan template and the sample Plan.
...
- [the 2024 CHR CSV/SAS Analytic Data Documentation](https://www.countyhealthrankings.org/sites/default/files/media/document/2024%20Analytic%20Documentation.pdf) file (PDF), and
...
This code should create a `chr_2024_raw` tibble with 3195 rows (observations) and 770 columns (variables). 

To complete Task 1, write code to restrict the `chr_2024_raw` data to include only the **3088** rows which have "county_clustered" values of 1, since the other rows will not be used by us in this project. 
...
At the end of Task 1, your `chr_2024_raw` tibble should have 3088 rows (counties) and 770 columns (variables).
...
After making your selection, and filtering the data, you should have a tibble called `chr_2024` which contains all of the 300-800 counties in your six states, and 770 columns. 

# Data Task 3. Select analytic variables

Next, you will select exactly nine variables (columns) from the 770 you should have at the start of Task 3. These nine variables must include the following four:
...
- The **brief label from CHR 2024** column in Table B are shown in the first (deleted in R) row in the raw .csv file for 2024, and are also specified in the [2024 CHR Analytic Data Documentation](https://www.countyhealthrankings.org/sites/default/files/media/document/2024%20Analytic%20Documentation.pdf) PDF file.
...
Create a new categorical variables (factor) based on your fourth variable, and add this new factor to your **chr_2024** tibble. You'll also retain the original (quantitative) version of this variable, so your tibble will now have 10 variables.
...
The data in the `431-projA-chr_2019.csv` file on our [431-data](https://github.com/THOMASELOVE/431-data) repository should be used to pull in the data from CHR 2019.

Assuming you have placed the `431-projA-chr_2019.csv` file in your R Project directory for Project A, then the following code should pull in the information you'll need into a tibble.

```{r}
#| eval: false
chr_2019_raw <- read_csv("431-projA-chr_2019.csv", 
...
Across all of the states you might have selected back in Data Task 3, each of these variables has information for **2,952** ranked counties in CHR 2024. However, **33** counties were ranked in CHR 2024 but not in CHR 2019. When we join the data in CHR 2024 to the data in CHR 2019, there are **33** counties with missing CHR 2019 data on these variables.

**The affected states** are: CA (3 counties with missing CHR 2019 data), CO (2), ID (1), KS (1), LA (7), MD (1), MI (1), MN (2), MO (2), MS (1), MT (3), PA (1), SD (1), TX (2), UT (1) and VA (4).
...
## Sources and Years for CHR 20019 Variables
...
------: | ---------------------- | --------------------------- | :-------:
**`v001`** | Premature death | National Center for Health Statistics - Mortality Files | 2014-16
**`v009`** | Adult smoking | Behavioral Risk Factor Surveillance System | 2016
**`v011`** | Adult obesity | CDC Diabetes Interactive Atlas | 2014
...
**`v060`** | Diabetes prevalence | CDC Diabetes Interactive Atlas | 2014
**`v070`** | Physical inactivity | CDC Diabetes Interactive Atlas | 2014
**`v139`** | Food insecurity | Map the Meal Gap	| 2015
