...
1. Use the `tabyl()` function from the `janitor` package to produce a two-way table of `state` (in the rows) classified against the factor variable (with levels Low and High, plus some missing values, in the columns) that you created in Data Task 4 from the [Data page](data.qmd).
...
2. Display the results of `describe_distribution()` from the `datawizard` package (from the `easystats` meta-package), run on the entire tibble. 
